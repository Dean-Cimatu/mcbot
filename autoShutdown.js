...
async function runHourlySnapshot(ssh, srv) {
  try {
    const result = await ssh.execCommand(`powershell -File "C:\\MinecraftServer\\backup.ps1" -serverId ${srv.id} -hourly`);
    if (result.stdout.includes('hourly_complete')) { addLog(`Hourly snapshot complete for ${srv.name}`); return true; }
    addLog(`Hourly snapshot failed for ${srv.name}: ${result.stdout}`); return false;
  } catch (err) { console.error(`Hourly snapshot error for ${srv.name}:`, err.message); return false; }
}

async function runShutdownBackup(ssh, srv) {
  try {
    const result = await ssh.execCommand(`powershell -File "C:\\MinecraftServer\\backup.ps1" -serverId ${srv.id} -shutdown`);
    if (result.stdout.includes('backup_complete')) { addLog(`Shutdown backup complete for ${srv.name}`); return true; }
    addLog(`Shutdown backup failed for ${srv.name}: ${result.stdout}`); return false;
  } catch (err) { console.error(`Shutdown backup error for ${srv.name}:`, err.message); return false; }
}

async function checkAllStoppedAndShutdown(client) {
  const srvs = await getServers();
  for (const s of srvs) { const on = await isPortOpen(process.env.PC_TAILSCALE_IP, s.port); if (on) return; }

  let pcStatus = { in_use: false };
  try { const res = await fetch(`http://${process.env.PC_TAILSCALE_IP}:${process.env.PC_STATUS_PORT}`); pcStatus = await res.json(); } catch {}

  const owner = await client.users.fetch(process.env.OWNER_ID);
  if (pcStatus.in_use) { owner.send(`All servers stopped. PC in use — leaving on.\nRun \`shutdown\` when done.`); addLog('PC in use — leaving on.'); return; }

  owner.send('All servers stopped. Shutting down PC.');
  addLog('All servers stopped. Shutting down PC.');
  let ssh;
  try { ssh = await getSSH(); await ssh.execCommand('shutdown /s /t 0'); }
  catch (err) { console.error('Shutdown SSH error:', err.message); }
  finally { try { if (ssh) ssh.dispose(); } catch {} }
}
...
async function startAutoShutdownLoop(client) {
  ...
  setInterval(async () => {
    ...
    } catch (err) { console.error('Hourly snapshot loop error:', err.message); }
    finally { try { if (ssh) ssh.dispose(); } catch {} }
  }, 60 * 60 * 1000);
  ...
  setInterval(async () => {
    try {
      ...
      for (const srv of srvs) {
        ...
        try {
          ...
          if (success) { owner.send(`✅ ${srv.name} backed up and stopped.`); addLog(`${srv.name} stopped and archived.`); }
          else { owner.send(`⚠️ ${srv.name} stopped. Archive failed but hourly snapshots preserved.`); addLog(`${srv.name} stopped, archive failed.`); }
    ...
    } catch (err) { console.error('Auto shutdown loop error:', err.message); }
  }, 5 * 60 * 1000);
}
